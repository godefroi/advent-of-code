...
public class Problem
{
	...
	[Test]
	[Arguments("1,9,10,3,2,3,11,0,99,30,40,50", "[3500,9,10,70,2,3,11,0,99,30,40,50]")]
	...
	public async Task SampleProgramsExecuteCorrectly(string program, string output)
	{
		var computer = new IntcodeComputer(program);

		await Assert.That(computer.Resume()).IsEqualTo(IntcodeComputer.InterruptType.Terminated);
		await Assert.That(computer.ToString()).IsEqualTo(output);
	}
}
